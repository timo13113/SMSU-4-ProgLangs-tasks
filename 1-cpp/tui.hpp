...
void transport_input(
    ...
    while (1) {
        ...
        while ((pos = input_transports.find(delimiter)) != std::string::npos)
        {
            ...
            {
                if (transport_names[i] == suggested_transport) {
        ...
        for (unsigned int i = 0; i < transport_names.size(); i++)
        {
            if (transport_names[i] == input_transports) {
                transport_whitelist.at(i) = transport_whitelist.at(i) xor true;
                was_found = true;
                good_transports.push_back(
...
void path_print(
    ...
    for (unsigned int i = 0; i < path.length(); i++)
    {
        ...
        cruise_str = 
            ...
            "(" + transport_names[path[i].second.transport_type_id] + 
...
void print_paths_a_b(
    ...
    clear();
    printw("Лучший путь из города %s в город %s:\n", 
        city_names[a].c_str(), city_names[b].c_str());
    log << "Лучший путь из города " << city_names[a] 
        << " в город " << city_names[b] << ":\n";
    
    path_print(found_paths.get_path_to(b), city_names, transport_names, log);
...
void print_paths_multiple(
    ...
    printw("Всего найдено %u путей в %u городов.", 
        num_visited-(int)!SHOW_TRIVIAL_PATHS, 
        num_visited-(int)!SHOW_TRIVIAL_PATHS);
    log << "Всего найдено " << num_visited-(int)!SHOW_TRIVIAL_PATHS << " путей в " 
        << num_visited-(int)!SHOW_TRIVIAL_PATHS << " городов.";
